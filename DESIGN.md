...
- Geist Sans — negative letter-spacing at display sizes for the signature "Apple tight" headline feel.
...
### Font Family
- **Display**: `Geist Sans, system-ui, -apple-system, sans-serif` — the project typeface for headlines, product copy, and interface chrome.
- **Body / UI**: `Geist Sans, system-ui, -apple-system, sans-serif` — the same family used for body copy, captions, buttons, and links for a simple unified landing page system.
...
| `{typography.display-md}` | 34px | 600 | 1.47 | -0.374px | Section heads (Geist Sans at display proportions) |
...
### Font Implementation
Geist Sans is the canonical project font. Load it once through Next font configuration, expose it as `--font-sans`, and use `font-sans` across landing and campaign pages. Keep the system fallback stack after Geist Sans for resilience.
...
**`button-primary`** — The signature action. Background `{colors.primary}` (Action Blue #0066cc), text `{colors.on-primary}` in `{typography.body}` (Geist Sans 17px / 400), rounded `{rounded.pill}` (full pill — capsule-shaped), padding 11px × 22px. Blue filled buttons must always render white text in default, hover, active, and focus states.
...
**`button-store-hero`** — A larger primary CTA used on hero surfaces. Same Action Blue + Paper White as `{component.button-primary}`, but with `{typography.button-large}` (18px / 300) and slightly more padding (14px × 28px). Blue filled buttons must always keep `{colors.on-primary}` text, including hover and pressed states.
...
### Do
- Use `{colors.primary}` (Action Blue #0066cc) for every interactive element — links, pill CTAs, focus signals — and nothing else. Any button with a blue background must use `{colors.on-primary}` (#ffffff) text in every state.
...
4. Document hover only when it changes contrast requirements, such as blue filled buttons preserving white text.
5. Display headlines stay Geist Sans 600 with negative letter-spacing. Body stays Geist Sans 400 at 17px. The boundary is unbreakable.
