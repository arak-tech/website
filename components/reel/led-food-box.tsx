...
import { QrImageFace } from './qr-image-face'
...
type LedFoodBoxProps = {
  spinning?: boolean
  winningIndex?: number
  /** Parent allows lid phase (win / brand); flap still waits until rotation stops */
  lidOpen?: boolean
}
...
export function LedFoodBox({
  ...
  useEffect(() => {
    const id = window.setTimeout(
      () => setFlapOpen(!spinning && lidOpen),
      spinning || !lidOpen ? 0 : CUBE_SETTLE_MS,
    )
          ...
          <motion.div className="box-lid-anchor" style={{ transformStyle: 'preserve-3d' }}>
            <div className="box-lid-anchor-inner">
              <div className="box-lid-flap">
                ...
                >
                  <span className="box-lid-kicker">OFFER READY</span>
                  {showFlapOpen && (
